...
static void warning_sim_callback(warn_btn_t btn_type);
...
void page_handler (void *p_arg)
{
	...
	else if (p_page_hdl->event == EVENT_SIGNAL(btn_rodar.id,EVENT_CLICK))
	{
		...
		else
		{
			...
			warn_args.func_callback = warning_rodar_callback;
	...
	else if (p_page_hdl->event == EVENT_SIGNAL(btn_sim.id,EVENT_CLICK))
	{
		...
		else
		{
			...
			warn_args.img_txt[1] = IMG_CONTINUAR;
			warn_args.msg_count = 2;
			warn_args.time_interval = TIME_SCREENS;
			warn_args.func_callback = warning_sim_callback;
			warning_page.p_args = &warn_args;
			mn_screen_change(&warning_page,EVENT_SHOW);
		}
	}
...
static void warning_sim_callback(warn_btn_t btn_type)
{
	switch (btn_type)
	{
		case BTN_PRESSED_SIM: mn_screen_change(&sim_page,EVENT_SHOW); break;
		case BTN_PRESSED_NAO: mn_screen_change(&auto_page,EVENT_SHOW);break;
	}
}
