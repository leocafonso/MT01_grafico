...
mn_screen_t zerarmaq_page = {.id 		 = SC_PAGE6,
					.wt_selected = 0,
					.name        = "cutPl",
					.p_widget = p_widget,
#if (TIMER_NUM > 0)
...
void page_attach (void *p_arg)
{
	...
	zerarmaq_page.iif_func[SC_KEY_ENTER] = desloca_key_enter;
	zerarmaq_page.iif_func[SC_KEY_ESC] = desloca_key_esc;
	zerarmaq_page.iif_func[SC_KEY_ZDOWN] = desloca_key_zdown;
	zerarmaq_page.iif_func[SC_KEY_ZUP] = desloca_key_zup;
	zerarmaq_page.iif_func[SC_KEY_RELEASE] = desloca_key_release;
}
...
void page_handler (void *p_arg)
{
	...
	{
		widgetVisible(&btn_play, NT_HIDE);
	...
	else if (p_page_hdl->event == EMERGENCIA_SIGNAL_EVENT)
	{
		...
		widgetChangePic(&btn_play, IMG_BTN_PLAY,IMG_BTN_PLAY_PRESS);
		machine_is_paused = true;
		emergencia_args.p_ret_page = &manual_page;;
